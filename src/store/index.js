...
export function getClientStore() {
  ...
  const combinedReducers = combineReducers(reducers)
	const store = applyMiddleware(thunk.withExtraArgument(clientRequest), promise, routerMiddleware, logger)(createStore)(combinedReducers, initialState)
...
export function getServerStore(req) {
	...
	const reducers = { counter, user, auth, router: routerReducer }
  const combinedReducers = combineReducers(reducers)
	const store = applyMiddleware(thunk.withExtraArgument(serverRequest(req)), promise, routerMiddleware, logger)(createStore)(combinedReducers)
	const history = createReduxHistory(store)
	return { store, history }
}